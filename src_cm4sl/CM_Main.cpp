...
{
    SimCore_Apo_SetAppClass("CarMaker");
...
DoReturn:
    apopx_reinit_apo(SimCore.Start.Ok);
    if (SimCore.Start.Ok) {
...
MainThread_Init(void)
{
    ...
    /*** load idle parameters/Test Run or start the selected Test Run */
    {
        ...
        SimCore_ApoMsg_Eval_SimStart(si->Project, si->User, si->Name, si->FName, si->Variation, si->Mode, si->SaveMode,
            si->OnErrSaveHist);
...
int
CM_Main_Begin(int argc, char **argv, struct ApopxInfo_t *apopxinfo, struct CMShlib_t *apopxtab)
{
    ...
    if (argv != NULL && (argv = SimCore_ScanCmdLine(&argc, argv)) != NULL
    ...
    if (SimCore.State != SCState_Start) {
        LogErrF(EC_Init, "CM_Main.cpp: Initialization failed to reach SCState '%s'." " Current SCState is '%s'",
            SimCore_State_Str(SCState_Start), SimCore_State_Str(SimCore.State));
        return -2;
    }
     ...
     * inconsistencies (e.g. invalid Info File pointers like SimCore.Vhcl.Inf).
     */
    ...
    for (CycleNo64 = 0; 1; CycleNo64++) {
        ...
        if (SimCore.NextState != SCState_Init && SimCore.NextState != SCState_Start
            && SimCore.NextState != SCState_StartWait && SimCore.NextState != SCState_StartWaitAnim
            && SimCore.NextState != SCState_StartSim && SimCore.NextState != SCState_StartLastCycle) {
            break;
        }
    }
...
CM_Main_End(void)
{
    ...
    if (CMLibInitState >= CMLib_ReadyForCycling) {
        ...
        for (CycleNo64 = 0; SimCore.State != SCState_Idle; CycleNo64++) {
            if (MainThread_BeginCycle(CycleNo64) != 0 || MainThread_DoCycle(CycleNo64, CyclePart_All) != 0) {
                break;
            }
            MainThread_FinishCycle(CycleNo64);
        }
...
# endif
void
mexFunction(int nlhs, struct mxArray_tag *plhs[], int nrhs, const struct mxArray_tag *prhs[])
{
    char const **s;
