...
static char const ThisModelClass[] = "Aero";
...
MyModel_New(tInfos *Inf, void *pCfg, char const *KindKey, char const *IdKey)
{
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Aero, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
{
    struct tMyModel *mp = (struct tMyModel *) MP;
    tAeroIF         *IF = (tAeroIF *) pIF;
    double           vel, x, cX[6];

    vel = sqrt(IF->ApproachVel_1[0] * IF->ApproachVel_1[0] + IF->ApproachVel_1[1] * IF->ApproachVel_1[1]);
