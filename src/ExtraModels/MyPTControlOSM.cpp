...
static char const ThisModelClass[] = "PowerTrain.ControlOSM";
...
{
    struct tPTControlOSM_CfgIF *CfgIF = (struct tPTControlOSM_CfgIF *) pCfgIF;
    struct tMyModel            *mp    = NULL;
    char                        MsgPre[64];
    char const                 *ModelKind;
    int                         VersionId = 0;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PTControlOSM, KindKey, 0, ThisVersionId, &VersionId))
        == NULL) {
        return NULL;
    }
...
{
    struct tMyModel  *mp = (struct tMyModel *) MP;
    tPTControlOSM_IF *IF = (tPTControlOSM_IF *) pIF;
    int               rv = 0;

    switch (IF->OperationState) {
        case (OperState_Absent):
            rv = MyModel_Calc_Absent(mp, IF, dt);
            break;

        case (OperState_PowerOff):
            rv = MyModel_Calc_PowerOff(mp, IF, dt);
            break;

        case (OperState_PowerAccessory):
            rv = MyModel_Calc_PowerAcc(mp, IF, dt);
            break;

        case (OperState_PowerOn):
            rv = MyModel_Calc_PowerOn(mp, IF, dt);
            break;

        case (OperState_Driving):
            rv = MyModel_Calc_Drive(mp, IF, dt);
            break;
    }
