...
static char const ThisModelClass[] = "PowerTrain.PTXWD";
...
{
    tPowerTrainXWD_CfgIF *CfgIF     = (tPowerTrainXWD_CfgIF *) pCfgIF;
    struct tMyModel      *mp        = NULL;
    int                   VersionId = 0;
    char                  MsgPre[64];
    char const           *ModelKind;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PowerTrainXWD, KindKey, 0, ThisVersionId, &VersionId))
        == NULL) {
        return NULL;
    }
    ...
    if (CfgIF->DriveLine.DriveSourcePos[0] != Diff_Front || CfgIF->DriveLine.DriveSourcePos[1] != NoPosition
        || CfgIF->DriveLine.DriveSourcePos[2] != NoPosition || CfgIF->DriveLine.DriveSourcePos[3] != NoPosition) {
        LogErrF(EC_Init, "%s: model supports only one drive source at the front differential", MsgPre);
        goto ErrorReturn;
    }
    ...
    if (CfgIF->GearBox[0].GBKind != GBKind_NoGearBox) {
...
{
    tPowerTrainXWD_IF          *IF    = (tPowerTrainXWD_IF *) pIF;
    tPowerTrainXWD_CfgIF const *CfgIF = IF->CfgIF;
#ifdef __cplusplus
