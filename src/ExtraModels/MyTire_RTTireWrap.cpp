...
static char const ThisModelKind[]  = "RTTireWrap";
...
MyModel_DeclQuants_dyn(struct tMyModel *mp, char const *Ident, int park)
{
    ...
    if (park) {
        memcpy(&MyModel_Dummy.Head, &mp->Head, sizeof(mp->Head));
        mp = &MyModel_Dummy;
    }
    ...
    sprintf(buf, "RTTireWrap.muFac_%s", Vehicle_TireNo_Str(Tire_TireNo(mp->Head.TireNo)));
...
MyModel_ParamsChanged(void *MP)
{
    ...
    if (mp->TireFile.path == NULL || mp->TireFile.ModTime != PathModTime(mp->TireFile.path)) {
        return 1;
    }
...
static void *
MyModel_New(struct tInfos *inf, char const *ParamDir, int TireNo, int Side, int Twin)
{
    ...
    if (sscanf(item, "CarMaker-Tire-%s %d", ModelKind, &VersionId) < 1) {
        LogErrF(EC_Init, "%s: Wrong syntax in tire kind " "information (FileIdent=%s, file '%s')", MsgPre, item, FName);
        goto ErrorReturn;
    }

    if (strcmp(ModelKind, ThisModelKind) != 0) {
        LogErrF(EC_Init, "%s: Tire with wrong Kind" " (got '%s', expected '%s %d', file '%s')", MsgPre, item,
            ThisModelKind, ThisVersionId, FName);
        goto ErrorReturn;
    }
    if (VersionId != ThisVersionId) {
        LogErrF(EC_Init, "%s: Tire model '%s' " "with wrong version (got %d, expected %d, file '%s')", MsgPre,
            ThisModelKind, VersionId, ThisVersionId, FName);
        goto ErrorReturn;
    }
    ...
    if (mp->Wrapped.New_InfDir3Int != NULL) {
        mp->Wrapped_param = mp->Wrapped.New_InfDir3Int(Inf_RTTire, ParamDir, TireNo, Side, Twin);
...
MyModel_Delete(void *MP, char const *Ident)
{
    ...
    if (mp->Wrapped_param != NULL && mp->Wrapped.Delete != NULL) {
        mp->Wrapped.Delete(mp->Wrapped_param, (char *) Ident);
    }
...
Tire_Register_MyModelRTTireWrap(void)
{
    ...
    m.New_InfDir3Int = MyModel_New;
    m.Calc           = MyModel_Calc;
    m.Delete         = MyModel_Delete;
    m.DeclQuants     = MyModel_DeclQuants;
    m.ParamsChanged  = MyModel_ParamsChanged;
