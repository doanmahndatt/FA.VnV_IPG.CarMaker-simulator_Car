...
{
    struct tPTEngineCU_CfgIF *CfgIF = (struct tPTEngineCU_CfgIF *) pCfgIF;
    struct tMyModel          *mp    = NULL;
    char const               *ModelKind;
    int                       VersionId = 0;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PTEngineCU, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
    ...
    /* FuelCutOff */
    mp->nFuelCutOff = iGetDblOpt(Inf, "PowerTrain.ECU.FuelCutOff", CfgIF->rotv_idle * 2.0 * radsec2rpm) * rpm2radsec;
...
{
    struct tMyModel         *mp    = (struct tMyModel *) MP;
    struct tPTEngineCU_IF   *IF    = (struct tPTEngineCU_IF *) pIF;
    tPTEngineCU_CfgIF const *CfgIF = IF->CfgIF;
