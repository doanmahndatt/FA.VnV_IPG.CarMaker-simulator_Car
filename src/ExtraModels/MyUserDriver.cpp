...
static char const ThisModelClass[] = "UserDriver";
static char const ThisModelKind[]  = "MyUserDriver";
...
MyModel_New(tInfos *Inf, void *pCfg, char const *KindKey, char const *IdKey)
{
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_UserDriver, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
{
    struct tMyModel      *mp = (struct tMyModel *) MP;
    struct tUserDriverIF *IF = (struct tUserDriverIF *) pIF;
