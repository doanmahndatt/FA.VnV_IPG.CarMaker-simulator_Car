...
static char const ThisModelClass[] = "Tire";
...
static char const ThisModelKind_CPI[] = "MyModelCPI";
static int const  ThisVersionId_CPI   = 1;

static void *
MyModel_CPI_New(struct tInfos *inf, char const *ParamDir, int TireNo, int Side, int Twin)
{
    ...
    int is3DTire = 0;   // set 0 if CPI interface is used
    if (Tire_New_Head(&mp->Head, inf, TireNo, Side, Twin, is3DTire) < 0) {
...
Calc_TireFrcTrq_W(struct tMyModel *mp, tTireIF *IF, double dt)
{
    ...
    /* Fx */
    abss          = fabs(IF->Slp);
    IF->Frc_W[0]  = IF->Frc_W[2] * M_SGN(IF->Slp) * mp->Fx.c1 * (1.0 - exp(-mp->Fx.c2 * abss)) - mp->Fx.c3 * abss;
    IF->Frc_W[0] *= IF->muRoad;

    /* Fy */
    absa          = fabs(sin(IF->Alpha));
    IF->Frc_W[1]  = -IF->Frc_W[2] * M_SGN(IF->Alpha) * mp->Fy.c1 * (1.0 - exp(-mp->Fy.c2 * absa)) - mp->Fy.c3 * absa;
...
static int
MyModel_CPI_Calc(void *MP, void *pIF, void *pIF2, double dt)
{
    tTireIF         *IF = (tTireIF *) pIF;
    struct tMyModel *mp = (struct tMyModel *) MP;

    if (IF->TireCalcMode == TireCalcPrepNoRoad || IF->TireCalcMode == TireCalcPrepWithRoad) {
...
Tire_Register_MyModelCPI(void)
{
    ...
    memset(&m, 0, sizeof(m));
    m.VersionId      = ThisVersionId_CPI;
    m.New_InfDir3Int = MyModel_CPI_New;
    m.Calc           = MyModel_CPI_Calc;
...
static char const ThisModelKind_STI[] = "MyModelSTI";
static int const  ThisVersionId_STI   = 1;

static void *
MyModel_STI_New(struct tInfos *inf, char const *ParamDir, int TireNo, int Side, int Twin)
{
    struct tMyModel *mp = NULL;
    char             MsgPre[64];
    ...
    int is3DTire = 1;   // set 1 if STI interface is used
    if (Tire_New_Head(&mp->Head, inf, TireNo, Side, Twin, is3DTire) < 0) {
        LogErrF(EC_Init, "%s: Failed to initialize the tire header struct", MsgPre);
        goto ErrorReturn;
    }
...
MyModel_STI_Calc_CP(tTParamHeader *th, tTireIF *IF)
{
    ...
    if ((IF->TireCalcMode == TireCalcPrepWithRoad || IF->TireCalcMode == TireCalcPrepWhlWithRoad
            || IF->TireCalcMode == TireCalcSim)
        && !Vehicle.ModelCheck.SPMM) {
    ...
    if (IF->onRoad == RPK_OffRoad && rv >= 0) {
        LogErrF(EC_Sim, "Vehicle leaves road at x=%g, y=%g TireNo=%d", rIn.xyz[0], rIn.xyz[1], th->TireNo);
    }
...
static int
MyModel_STI_Calc(void *MP, void *pIF, void *pIF2, double dt)
{
    tTireIF         *IF = (tTireIF *) pIF;
    struct tMyModel *mp = (struct tMyModel *) MP;
...
Tire_Register_MyModelSTI(void)
{
    ...
    memset(&m, 0, sizeof(m));
    m.VersionId      = ThisVersionId_STI;
    m.New_InfDir3Int = MyModel_STI_New;
    m.Calc           = MyModel_STI_Calc;
    m.Delete         = MyModel_Delete;
    m.DeclQuants     = MyModel_DeclQuants;
    /* Should only be used if the model doesn't read params from extra files */
    m.ParamsChanged = ParamsChanged_IgnoreCheck;
