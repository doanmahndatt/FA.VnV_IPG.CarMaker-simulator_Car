...
static char const ThisModelClass[] = "Susp_Spring";
...
MyModel_New(struct tInfos *Inf, void *pCfgIF, char const *KindKey, char const *IdKey)
{
    ...
    tSusp_SpringCfgIF *CfgIF = (tSusp_SpringCfgIF *) pCfgIF;
    int                n, VersionId = 0;
    char               MsgPre[64];
    char const        *ModelKind;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Susp_Spring, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
{
    tSusp_SpringIF  *IF = (tSusp_SpringIF *) pIF;
    struct tMyModel *mp = (struct tMyModel *) MP;

    IF->Force = mp->Spring.dFrc_dl * (mp->Spring.len0 - IF->length) + mp->Spring.Frc0;
