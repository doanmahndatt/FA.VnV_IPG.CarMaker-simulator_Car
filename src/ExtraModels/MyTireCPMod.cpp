...
static void *
MyModel_New(struct tInfos *inf, char const *Dir, int TireNo, int Side, int Twin)
{
...
{
    struct tMyModel *mp = (struct tMyModel *) MP;
    tTireCPModIF    *IF = (tTireCPModIF *) pIF;
...
static char const ThisModelKind_FP[] = "MyModel_FourPoster";
static int const  ThisVersionId_FP   = 1;
...
MyModel_DeclQuants_dyn_FP(struct tMyModel_FP *mp, int park)
{
    ...
    char const *buft = mp->Twin ? ".Twin" : "";

    /* Define here dict entries for dynamically allocated variables. */
    df = DDefaultCreate(NULL);

    int           VhclNo    = Tire_No2VhclNo(mp->TireNo);
    tVhclClass_Id vhclclass = TireNo_VhclClass_Id(mp->TireNo);

    if (vhclclass == VhclClass_Trailer_Id) {
        char buf[8];
        Gen_No2Prefix(buf, ObjIdType_Trailer, VhclNo, prefix_short_capitalized);
        DDefPrefix(df, "MyTireCPMod_FP.%s.%s%s.", buf, Tire_TireNo_Str(mp->TireNo), buft);
    } else {
...
static void
MyModel_DeclQuants_FP(void *MP, char const *Ident)
{
...
static void *
MyModel_New_FP(struct tInfos *inf, char const *Dir, int TireNo, int Side, int Twin)
{
...
static int
MyModel_Calc_FP(void *MP, void *pIF, void *pIF2, double dt)
{
    struct tMyModel_FP *mp = (struct tMyModel_FP *) MP;
    tTireCPModIF       *IF = (tTireCPModIF *) pIF;
...
static void
MyModel_Delete_FP(void *MP, char const *Ident)
{
    struct tMyModel_FP *mp = (struct tMyModel_FP *) MP;
...
TireCPMod_Register_MyModelFourPoster(void)
{
    ...
    memset(&m, 0, sizeof(m));
    m.VersionId      = ThisVersionId_FP;
    m.New_InfDir3Int = MyModel_New_FP;
    m.Calc           = MyModel_Calc_FP;
    m.Delete         = MyModel_Delete_FP;
    m.DeclQuants     = MyModel_DeclQuants_FP;
    /* Should only be used if the model doesn't read params from extra files */
    m.ParamsChanged = ParamsChanged_IgnoreCheck;
