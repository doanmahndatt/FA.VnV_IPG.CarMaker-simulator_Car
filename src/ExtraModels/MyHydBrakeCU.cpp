...
static char const ThisModelClass[] = "Brake.Control";
 ...
 *
 * Call:
 * - one times at the beginning of every Test Run
 */

static void *
MyModel_New(tInfos *Inf, void *pCfgIF, char const *KindKey, char const *IdKey)
{
    tHydBrakeCU_CfgIF *CfgIF = (tHydBrakeCU_CfgIF *) pCfgIF;
    ...
    if (CfgIF->nWheels != NWHEEL) {
        LogErrF(EC_Init, "%s: wheel number mismatch (brake %d, vehicle %d)", MsgPre, NWHEEL, CfgIF->nWheels);
        return NULL;
    }

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_HydBrakeControl, KindKey, 0, ThisVersionId, &VersionId))
        == NULL) {
        return NULL;
    }
 ...
 *
 * Call:
 * - at the beginning of every Test Run, after the call for the _New function
 */
...
static int
MyModel_Calc(void *MP,     /* model parameter handle	*/
    void          *pIF,    /* brake interface structure	*/
    void *pIF2, double dt) /* time step			*/
{
    int                    i;
    double                 desiredTrq[NWHEEL], dTrq[NWHEEL];
    struct tMyModel       *mp = (struct tMyModel *) MP;
    struct tHydBrakeCU_IF *IF = (struct tHydBrakeCU_IF *) pIF;
