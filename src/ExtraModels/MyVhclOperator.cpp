...
static char const ThisModelClass[] = "DrivMan.VhclOperator";
...
{
    struct tMyModel              *mp       = (struct tMyModel *) MP;
    struct tVhclOperatorPreSimIF *PreSimIF = (struct tVhclOperatorPreSimIF *) pPreSimIF;
    struct tVhclOperatorIF       *IF       = (struct tVhclOperatorIF *) pIF;

    /* Take Test Run start conditions only for state Driving */
...
{
    struct tMyModel *mp = (struct tMyModel *) MP;
    tVhclOperatorIF *IF = (tVhclOperatorIF *) pIF;

    if (IF->OperationState != mp->OperationState_last || IF->OperationState_trg != mp->OperationState_trg_last) {
        mp->Phase = 0;
    }
...
{
    struct tVhclOperatorCfgIF *CfgIF = (struct tVhclOperatorCfgIF *) pCfgIF;
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_VhclOperator, KindKey, 0, ThisVersionId, &VersionId))
        == NULL) {
        return NULL;
    }
