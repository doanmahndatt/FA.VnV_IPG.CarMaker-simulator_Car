...
#include "MySuspKnC.cpp"

static char const ThisModelKindAuxFMU[] = "_MyModel_AuxFMU";
...
static void *
MyModel_New_AuxFMU(tInfos *Inf, void *pSuspCfgIF, char const *KindKey, char const *Pre)
{
...
static int
MyModel_Calc_AuxFMU(void *MP, void *pIFMain, void *pIFOpp, double dt)
{
...
static void
MyModel_Delete_AuxFMU(void *MP, char const *Ident)
{
    /* Do not change */
    tAuxData *mp = (struct tAuxData *) MP;
    ...
    /* Replace this block with user model if FMU other than MySuspKnC_FMU is used */
    {
        ...
        MyModel_Delete(mp->MyAuxModel, Ident);
    }
}
...
Susp_KnC_Register_MyModel_AuxFMU(void)
{
    ...
    m.New       = MyModel_New_AuxFMU;
    m.Calc      = MyModel_Calc_AuxFMU;
    m.Delete    = MyModel_Delete_AuxFMU;
