...
static char const ThisModelClass[] = "Susp_TopMount";
...
MyModel_New(struct tInfos *Inf, void *pCfgIF, char const *KindKey, char const *IdKey)
{
    ...
    tSusp_TopMountCfgIF *CfgIF     = (tSusp_TopMountCfgIF *) pCfgIF;
    int                  VersionId = 0;
    char                 MsgPre[64];
    char const          *ModelKind;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Susp_TopMount, KindKey, 0, ThisVersionId, &VersionId))
        == NULL) {
        return NULL;
    }
...
{
    struct tMyModel  *mp = (struct tMyModel *) MP;
    tSusp_TopMountIF *IF = (tSusp_TopMountIF *) pIF;
    int               i;
    int const         OSRate = 10;
