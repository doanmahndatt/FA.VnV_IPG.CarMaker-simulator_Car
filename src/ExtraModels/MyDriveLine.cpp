...
static char const ThisModelClass[] = "PowerTrain.DL";
...
MyModel_New(struct tInfos *Inf, void *pCfgIF, char const *KindKey, char const *IdKey)
{
    ...
    tPTDriveLineCfgIF *CfgIF = (tPTDriveLineCfgIF *) pCfgIF;
    char               MsgPre[64];
    char const        *ModelKind;
    int                iS, VersionId = 0;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PTDriveLine, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
    ...
    if (CfgIF->DriveSourcePos[0] != Diff_Front || CfgIF->DriveSourcePos[1] != NoPosition
        || CfgIF->DriveSourcePos[2] != NoPosition || CfgIF->DriveSourcePos[3] != NoPosition) {
...
MyModel_Calc(void *MP, void *pIF, void *pIF2, double dt)
{
    ...
    tPTDriveLineIF          *IF    = (tPTDriveLineIF *) pIF;
    tPTDriveLineCfgIF const *CfgIF = IF->CfgIF;
