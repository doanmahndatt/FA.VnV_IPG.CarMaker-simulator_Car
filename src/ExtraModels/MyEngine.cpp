...
static char const ThisModelClass[] = "PowerTrain.Engine";
...
MyModel_New(struct tInfos *Inf, void *pCfgIF, char const *KindKey, char const *Ident)
{
    ...
    tPTEngineCfgIF  *CfgIF = (tPTEngineCfgIF *) pCfgIF;
    char             MsgPre[64];
    int              VersionId = 0;
    char const      *ModelKind;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PTEngine, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
{
    struct tMyModel *mp = (struct tMyModel *) MP;
    tPTEngineIF     *IF = (tPTEngineIF *) pIF;
