...
static char const ThisModelClass[] = "Susp_SpringSystem";
...
{
    tSusp_SpringSystemCfgIF *CfgIF = (tSusp_SpringSystemCfgIF *) pCfgIF;
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Susp_SpringSystem, KindKey, 0, ThisVersionId, &VersionId))
        == NULL) {
        return NULL;
    }
...
{
    tSusp_SpringSystemIF *IF = (tSusp_SpringSystemIF *) pIF;
    struct tMyModel      *mp = (struct tMyModel *) MP;
    int                   iS;

    for (iS = 0; iS < N_SUSPENSIONS; iS++) {
        IF->Force[iS] = mp->Spring[iS].dFrc_dl * (mp->Spring[iS].len0 - IF->length[iS]) + mp->Spring[iS].Frc0;
    }
