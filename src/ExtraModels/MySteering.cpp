...
static char const ThisModelClass[] = "Steering";
static char const ThisModelKind[]  = "MyModel";
static int const  ThisVersionId    = 1;

struct tMyModel {
    int    disabled;     /* error occurred, steering system disabled */
...
MyModel_New(tInfos *Inf, void *pCfgIF, char const *KindKey, char const *IdKey)
{
    ...
    tSteeringCfgIF  *CfgIF = (tSteeringCfgIF *) pCfgIF;
    double           val, dvec[3];
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Steering, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
    ...
    if ((val = iGetDblOpt(Inf, key, 0.0)) != 0.0) {
        ...
        LogErrF(EC_Init, "%s: Steering ratio rack/wheel '%s' not defined or zero.", MsgPre, key);
        mp->iSteer2Rack = 0.01;
    }
...
{
    struct tMyModel *mp = (struct tMyModel *) MP;
    tSteeringIF     *IF = (tSteeringIF *) pIF;
    double           mass;
	...
	*/
        ...
        /*** DOF rack position */
        mp->qpp = (mp->TrqAmplify * IF->Trq * mp->iRack2Steer + mp->RackFrc - mp->d * mp->qp) / mass;
	...
	*/
        ...
        IF->Trq = (mp->qpp * mass + mp->d * mp->qp - mp->RackFrc) * mp->iSteer2Rack / mp->TrqAmplify;
    }
