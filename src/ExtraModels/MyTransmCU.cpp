 ...
 * Use this model together with the IPG "Closed" Clutch and the
 * IPG "Automatic with Converter" GearBox.
 * The lock-up clutch of the converter isn't controlled in this model.
 *
 * For use with Examples/ExtraModels/DemoCar_MyPTTransmCU
 *
...
static char const ThisModelClass[] = "PowerTrain.TCU";
...
{
    struct tPTTransmCU_CfgIF *CfgIF = (struct tPTTransmCU_CfgIF *) pCfgIF;
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PTTransmCU, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
    ...
    if (CfgIF->GearBox[0].GBKind != GBKind_AutoWithManual || CfgIF->GearBox[0].ClKind != ClKind_Converter) {
...
{
    struct tMyModel       *mp = (struct tMyModel *) MP;
    struct tPTTransmCU_IF *IF = (struct tPTTransmCU_IF *) pIF;
#ifdef __cplusplus
    struct tPTTransmCU_IF::tPTTransmCU_IF_GearBoxOut *GBOut = &IF->GearBoxOut[0];
    struct tPTTransmCU_IF::tPTTransmCU_IF_GearBoxIn  *GBIn  = &IF->GearBoxIn[0];
#else
    struct tPTTransmCU_IF_GearBoxOut *GBOut = &IF->GearBoxOut[0];
    struct tPTTransmCU_IF_GearBoxIn  *GBIn  = &IF->GearBoxIn[0];
#endif
    tPTTransmCU_CfgIF const *CfgIF = IF->CfgIF;
    ...
    if (GBOut->GearNoTrg != NOTSET) {
        mp->GearNoTrg = M_BOUND(-CfgIF->GearBox[0].nBGears, CfgIF->GearBox[0].nFGears, GBOut->GearNoTrg);
    }
    ...
    if (IF->SelectorCtrl != SelectorCtrl_M && GBOut->GearNoTrg == NOTSET) {
        ...
        if (IF->SelectorCtrl == SelectorCtrl_D) { /* driving forwards */
            if (GBIn->GearNo > 0) {               /* already driving forwards */
                /* upshifting */
                if ((GBIn->GearNo < CfgIF->GearBox[0].nFGears) && (RotvRef >= nUp)) {
            ...
            if (GBIn->GearNo < 0) {                      /* already driving backwards */
                if ((-GBIn->GearNo < CfgIF->GearBox[0].nBGears) && (RotvRef >= nUp)) {
                    mp->GearNoTrg = GBIn->GearNo - 1;
                }
                if ((-GBIn->GearNo > 1) && (RotvRef <= nDown)) {
                    mp->GearNoTrg = GBIn->GearNo + 1;
