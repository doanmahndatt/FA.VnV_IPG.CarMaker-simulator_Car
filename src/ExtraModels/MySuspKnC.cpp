...
static char const ThisModelClass[]      = "SuspKnC";
static char const ThisModelKind[]       = "MyModel";
static char const ThisModelKindLR[]     = "MyModel_FLR";
static char const ThisModelKindFrcCpl[] = "MyModel_FrcCpl_R";
...
static void *
MyModel_New(struct tInfos *Inf, void *pSuspCfgIF, char const *KindKey, char const *Pre)
{
    struct tSuspCfgIF *SuspCfgIF = (struct tSuspCfgIF *) pSuspCfgIF;
    struct tMyModel   *mp        = NULL;
    char               MsgPre[64];
    char const        *ModelKind;
    int                VersionId = 0;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_SuspKnC, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
static void *
MyModel_New_LR(struct tInfos *Inf, void *pSuspCfgIF, char const *KindKey, char const *Pre)
{
    struct tSuspCfgIF *SuspCfgIF = (struct tSuspCfgIF *) pSuspCfgIF;
    struct tMyModel   *mp        = NULL;
    char               MsgPre[64];
...
static void *
MyModel_New_FrcCpl(struct tInfos *Inf, void *pSuspCfgIF, char const *KindKey, char const *Pre)
{
    struct tSuspCfgIF *SuspCfgIF = (struct tSuspCfgIF *) pSuspCfgIF;
    struct tMyModel   *mp        = NULL;
...
static int
MyModel_Calc(void *MP, void *pIFMain, void *pIFOpp, double dt)
{
    struct tMyModel *mp     = (struct tMyModel *) MP;
    tSuspIF         *IFMain = (tSuspIF *) pIFMain;
    int              i;
    ...
    for (i = 0; i < ixKinMax; i++) {
        IFMain->Kin[i] = IFMain->dqComp[i] * IFMain->qComp + IFMain->dqSteer[i] * IFMain->qSteer;
    }
...
static int
MyModel_Calc_LR(void *MP, void *pIFMain, void *pIFOpp, double dt)
{
    struct tMyModel *mp     = (struct tMyModel *) MP;
    tSuspIF         *IFMain = (tSuspIF *) pIFMain;
    tSuspIF         *IFOpp  = (tSuspIF *) pIFOpp;
    int              i;
    ...
    for (i = 0; i < ixKinMax; i++) {
        IFOpp->Kin[i] = IFOpp->dqComp[i] * IFOpp->qComp + IFOpp->dqSteer[i] * IFOpp->qSteer;
    }
...
static int
MyModel_Calc_FrcCpl(void *MP, void *pIFMain, void *pIFOpp, double dt)
{
    ...
    if (dt == 0 || AppStartInfo.ModelCheck) {
        SuspMod_FrcSpring_Calc(IFMain->CfgIF->SuspNo, IFMain->Kin[ixSpring], 0, &IFMain->FrcSpring);
...
Susp_KnC_Register_MyModel_LR(void)
{
    ...
    memset(&m, 0, sizeof(m));
    m.VersionId  = ThisVersionId;
    m.New        = MyModel_New_LR;
    m.Calc       = MyModel_Calc_LR;
    m.Delete     = MyModel_Delete;
    m.DeclQuants = MyModel_DeclQuants;
    /* Should only be used if the model doesn't read params from extra files */
    m.ParamsChanged = ParamsChanged_IgnoreCheck;
...
Susp_KnC_Register_MyModel_FrcCpl(void)
{
    ...
    memset(&m, 0, sizeof(m));
    m.VersionId  = ThisVersionId;
    m.New        = MyModel_New_FrcCpl;
    m.Calc       = MyModel_Calc_FrcCpl;
    m.Delete     = MyModel_Delete;
    m.DeclQuants = MyModel_DeclQuants;
    /* Should only be used if the model doesn't read params from extra files */
    m.ParamsChanged = ParamsChanged_IgnoreCheck;
