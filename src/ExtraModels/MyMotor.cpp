...
static char const ThisModelClass[] = "Powertrain.Motor";
...
static void MyModel_Delete(void *MP, char const *Ident);
...
MyModel_New(struct tInfos *Inf, void *pCfgIF, char const *KindKey, char const *Ident)
{
    ...
    tPTMotorCfgIF   *CfgIF = (tPTMotorCfgIF *) pCfgIF;
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PTMotor, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
{
    struct tMyModel     *mp    = (struct tMyModel *) MP;
    tPTMotorIF          *IF    = (tPTMotorIF *) pIF;
    tPTMotorCfgIF const *CfgIF = IF->CfgIF;
