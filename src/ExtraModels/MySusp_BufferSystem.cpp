...
static char const ThisModelClass[] = "Susp_BufferSystem";
...
{
    tSusp_BufferSystemCfgIF *CfgIF = (tSusp_BufferSystemCfgIF *) pCfgIF;
    struct tMyModel         *mp    = NULL;
    int                      iS, n, VersionId = 0;
    char                     MsgPre[64];
    char const              *ModelKind;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Susp_BufferSystem, KindKey, 0, ThisVersionId, &VersionId))
        == NULL) {
        return NULL;
    }
...
{
    tSusp_BufferSystemIF *IF = (tSusp_BufferSystemIF *) pIF;
    struct tMyModel      *mp = (struct tMyModel *) MP;
    int                   iS;
