...
static char const ThisModelClass[] = "PowerTrain.Control";
...
{
    struct tPTControlCfgIF *CfgIF = (struct tPTControlCfgIF *) pCfgIF;
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PTControl, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
{
    struct tMyModel *mp = (struct tMyModel *) MP;
    tPTControlIF    *IF = (tPTControlIF *) pIF;
...
OutOfOperState:
    ...
    if (mp->GBKind == GBKind_Manual) {
        IF->ClutchOut[0].Pos = IF->Clutch;
    }

    /* GearNo */
    if (mp->GBKind == GBKind_Manual) {
        IF->GearBoxOut[0].GearNoTrg = IF->GearNoTrg;
    } else if (mp->GBKind == GBKind_AutoWithManual && IF->SelectorCtrl == SelectorCtrl_M
               && IF->OperationState == OperState_Driving) {
        IF->GearBoxOut[0].GearNoTrg = IF->GearNoTrg;
    } else {
        IF->GearBoxOut[0].GearNoTrg = NOTSET;
    }
