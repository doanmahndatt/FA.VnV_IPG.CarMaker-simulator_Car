...
static char const ThisModelClass[] = "PowerTrain";
...
MyModel_New(tInfos *Inf, void *pCfgIF, char const *KindKey, char const *IdKey)
{
    ...
    tPowerTrainCfgIF *CfgIF = (tPowerTrainCfgIF *) pCfgIF;
    int               i, VersionId = 0;
    char              MsgPre[64];
    char const       *ModelKind;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PowerTrain, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
{
    int                  iS;
    tPowerTrainPreSimIF *PreSimIF = (tPowerTrainPreSimIF *) pPreSimIF;
    tPowerTrainIF       *IF       = (tPowerTrainIF *) pIF;
...
MyModel_Calc(void *MP, void *pIF, void *pIF2, double dt)
{
    ...
    /* Wheel accelerations */
    for (i = 0; i < NWHEEL; i++) {
        mp->Whl[i].rota = (IF->WheelOut[i].Trq_Drive + Trq_Ext2W[i]) / mp->Whl[i].Irot_act;
    }
    ...
    IF->SelectorCtrl_Out = IF->SelectorCtrl_In;
