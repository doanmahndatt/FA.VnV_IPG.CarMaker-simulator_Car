...
static char const ThisModelKind[] = "MyCar";
...
static struct {
    ...
    struct {
        tRoadEval *RE;
        int        LinkObjId, onJunction;
        int        JuncObjId, nextJuncObjId;
        double     s2nextJunc, s2lastJunc;
        double     Dist;
        double     X_H[3];
        double     Y_H[3];
        double     Z_H[3];
...
/* Vehicle tire data.
   Tires will always be initialized according to the Test Run/vehicle dataset,
...
static void
VehicleModel_DeclQuants(void *MP, char const *Ident)
{
}
...
Car2Vehicle(void)
{
    ...
    /* Information of location on road network (to be considered by autonomous traffic) */
    Vehicle.Road.LinkObjId     = Car.Road.LinkObjId;
    Vehicle.Road.onJunction    = Car.Road.onJunction;
    Vehicle.Road.JuncObjId     = Car.Road.JuncObjId;
    Vehicle.Road.nextJuncObjId = Car.Road.nextJuncObjId;
    Vehicle.Road.s2nextJunc    = Car.Road.s2nextJunc;
    Vehicle.Road.s2lastJunc    = Car.Road.s2lastJunc;
...
static int
carRoadEval_ChangeRoute(void *module, tObjId const newRoute, double const st[2])
{
    if (module == NULL) {
        return -1;
    }
    ...
    if (newRoute < 0) {
        LogErrF(EC_Sim, "MyCar: works only on route.");
        return -1;
    }

    struct tRoadEval *RoadEval = (struct tRoadEval *) module;
    RoadResetRoadEval(RoadEval, Env.Road);

    if (RoadEvalSetRouteByObjId(RoadEval, newRoute, 0) != ROAD_Ok) {
        LogErrF(EC_Init, "MyCar: failed to select route %d.", newRoute);
        return -1;
    }
...
static void *
VehicleModel_New(struct tInfos *Inf, void *pCfgIF, char const *Key1, char const *Key2)
{
    struct tVehicleCfg       *Cfg   = &Vehicle.Cfg;
    struct tUserVehicleCfgIF *CfgIF = (tUserVehicleCfgIF *) pCfgIF;
    int                       i     = 0, n, outMask;

    /* Get vehicle configuration parameters from vehicle Info File */
    ...
    if (!CfgIF->soft) {
        ...
        for (TireNo = 0; TireNo < Vehicle.Cfg.nWheels; TireNo++) {
            int const side = TireNo % 2 == 0 ? 1 : -1; /* 1 left, -1 right */
        ...
        BdyFrame_GenRegister(ObjIdType_Vehicle, 0, Vehicle_Fr1A, &Vehicle.Fr1A);
    ...
    outMask = ROAD_OT_MIN | ROAD_OT_SUV | ROAD_OT_TUV | ROAD_OT_NUV | ROAD_OT_EXT;
    ...
    /* register observer for route change */
    {
        tRouteChangeObserver observer;
        observer.module = (void *) Car.Road.RE;
        observer.fct    = carRoadEval_ChangeRoute;
        if (DrivMan_RouteChangeObserver_New(&observer) < 0) {
            LogErrF(EC_Init, "MyCar: failed to register road handle for route change");
            return NULL;
        }
    }

    // Get the vehicle position at the start of the new simulation
    SkipPreprocessing = iGetBoolOpt(Inf, "Vehicle.SkipPreprocessing", 1);
    ...
    /* Disable postprocessing after end of simulation, i.e. DON'T pilot the
       ...
       Should usually be set in the SimParameters file. Setting SimCore.GetIdle.Skip = 0 manually
       overwrites the SimParameters for all models in the current simulation. */
    // SimCore.GetIdle.Skip = 0;
    ...
    if (SkipPreprocessing) {
	   ...
	   function only comes afterwards when not taking part in preprocessing. */
        Car.PoI.x  = DrivMan.Cfg.RoadPos[0];
        Car.PoI.y  = DrivMan.Cfg.RoadPos[1];
        Car.PoI.z  = DrivMan.Cfg.RoadPos[2];
        Car.Fr1.vx = DrivMan.Cfg.Velocity * DrivMan.Cfg.RoadDir[0];
        Car.Fr1.vy = DrivMan.Cfg.Velocity * DrivMan.Cfg.RoadDir[1];
    }
...
static int
VehicleModel_Calc(void *MP, void *pIF, void *pIF2, double dt)
{
    double              val;
    char const         *msgpre = "MyCar: ";
    tRoadRouteIn        rIn;
    tRoadRouteOutExt    rOut;
    struct tVehicleCfg *Cfg = &Vehicle.Cfg;
    ...
    if (RoadRouteEvalExt(Car.Road.RE, msgpre, RIT_XY_S, &rIn, &rOut) != ROAD_Ok) {
        goto ErrorReturn;
    }
    if (!rOut.onRoad) {
        LogErrF(EC_Sim, "MyCar leaves road at about sRoad=%g m, x=%g, y=%g", Car.Road.Dist, rOut.xyz[0], rOut.xyz[1]);
        goto ErrorReturn;
    }

    Car.Fr1.z              = rOut.xyz[2];
    Car.Road.Dist          = rOut.st[0];
    Car.Road.LinkObjId     = rOut.linkObjId;
    Car.Road.onJunction    = rOut.onJunction;
    Car.Road.JuncObjId     = rOut.juncObjId;
    Car.Road.nextJuncObjId = rOut.nextJuncObjId;
    Car.Road.s2nextJunc    = rOut.nextJuncDist;
    Car.Road.s2lastJunc    = rOut.lastJuncDist;
    ...
    /*** engine torque */
    {
        ...
        dTrq_dn = (Car.PT.Trq[i] - Car.PT.Trq[i - 1]) / (Car.PT.rotp[i] - Car.PT.rotp[i - 1]);
    ...
    {
        double const vCritical = 2000.0 / 3.6;
        double       facBrake  = Car.Fr1.vx_1 <= 0 ? 0.0 : 1.0;
        Car.Fr1.ax_1           = Car.PT.Engine_Trq * Car.PT.iAct / Car.Whl.Radius / Car.Bdy1.mass
                     - 40. * Car.Driver.Brake * facBrake;

        Car.Fr1.ay_1 = Car.Fr1.vx_1 * Car.Fr1.vx_1 * Car.Steering.Ang / Car.Steering.i
                     * (1.0 / (Car.Info.WhlBaseF - Car.Info.WhlBaseR) - Car.Fr1.vx_1 / vCritical);
    }

    /*** longitudinal and side slip */
    val = 1.2 * Car.Fr1.ax_1 / Car.Tyres.ax_max;
    if (val > 1.) {
        Car.Tyres.LongSlipF = 0.05 + 10. * (val - 1.0);
    } else if (val < -1.) {
        Car.Tyres.LongSlipF = -0.05 + 10. * (val + 1.0);
    } else {
        Car.Tyres.LongSlipF = 0.05 * asin(val);
    }

    if (Car.Tyres.LongSlipF > 1.) {
        Car.Tyres.LongSlipF = 1.0;
    } else if (Car.Tyres.LongSlipF < -1.) {
        Car.Tyres.LongSlipF = -1.0;
    }
    ...
    if (val > 1.) {
        Car.Tyres.SideSlipAngF = 0.05 + 10. * (val - 1.0);
    } else if (val < -1.) {
        Car.Tyres.SideSlipAngF = -0.05 + 10. * (val + 1.0);
    } else {
        Car.Tyres.SideSlipAngF = 0.05 * asin(val);
    }

    if (Car.Tyres.SideSlipAngF > M_PI / 2.) {
        Car.Tyres.SideSlipAngF = M_PI / 2.;
    } else if (Car.Tyres.SideSlipAngF < -M_PI / 2.) {
        Car.Tyres.SideSlipAngF = -M_PI / 2.;
    }
    Car.Tyres.SideSlipAngR = Car.Tyres.SideSlipAngF;

    /* tyre force limitation */
    if (Car.Fr1.ay_1 > Car.Tyres.ay_max) {
        Car.Fr1.ay_1 = Car.Tyres.ay_max;
    }
    if (Car.Fr1.ay_1 < -Car.Tyres.ay_max) {
        Car.Fr1.ay_1 = -Car.Tyres.ay_max;
    }

    if (Car.Fr1.ax_1 > Car.Tyres.ax_max) {
        Car.Fr1.ax_1 = Car.Tyres.ax_max;
    }
    if (Car.Fr1.ax_1 < -Car.Tyres.ax_max) {
        Car.Fr1.ax_1 = -Car.Tyres.ax_max;
    }
    ...
    Car.Fr1.ax = Car.Fr1.ax_1 * cos(Car.Fr1.Yaw) - Car.Fr1.ay_1 * sin(Car.Fr1.Yaw);
    Car.Fr1.ay = Car.Fr1.ax_1 * sin(Car.Fr1.Yaw) + Car.Fr1.ay_1 * cos(Car.Fr1.Yaw);
...

    {
        ...
        Pos01PoI_0[0] = Vehicle.Cfg.Bdy1_CoM[0] * cos(Car.Fr1.Yaw) - Vehicle.Cfg.Bdy1_CoM[1] * sin(Car.Fr1.Yaw);
        Pos01PoI_0[1] = Vehicle.Cfg.Bdy1_CoM[0] * sin(Car.Fr1.Yaw) + Vehicle.Cfg.Bdy1_CoM[1] * cos(Car.Fr1.Yaw);
    ...
    NEV2FreiZYX(Car.Road.X_H, Car.Road.Y_H, Car.Road.Z_H, &Car.Fr1.rx, &Car.Fr1.ry, &val);
...
static void
VehicleModel_Delete(void *MP, char const *Soft)
{
    int TireNo;

    int soft = strtol(Soft, NULL, 10);
    ...
    // Reset of travelled distance needed
    Car.Info.Dist = 0.0;
...
Vehicle_Register_MyModel(void)
{
    ...
    m.VersionId       = 1;
    m.CompatVersionId = 1;
    m.New             = VehicleModel_New;
    m.Calc            = VehicleModel_Calc;
    m.Delete          = VehicleModel_Delete;
    m.DeclQuants      = VehicleModel_DeclQuants;
    /* Should only be used if the model doesn't read params from extra files */
    m.ParamsChanged = ParamsChanged_IgnoreCheck;

    return Model_Register(ModelClass_Vehicle, ThisModelKind, &m);
}
