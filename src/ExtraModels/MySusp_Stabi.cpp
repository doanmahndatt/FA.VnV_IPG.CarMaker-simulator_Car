...
static char const ThisModelClass[] = "Susp_Stabi";
...
{
    tSusp_StabiCfgIF *CfgIF     = (tSusp_StabiCfgIF *) pCfgIF;
    struct tMyModel  *mp        = NULL;
    int               VersionId = 0;
    char              MsgPre[64];
    char const       *ModelKind;

    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_Susp_Stabi, KindKey, 0, ThisVersionId, &VersionId)) == NULL) {
        return NULL;
    }
...
{
    tSusp_StabiIF   *IF = (tSusp_StabiIF *) pIF;
    struct tMyModel *mp = (struct tMyModel *) MP;
