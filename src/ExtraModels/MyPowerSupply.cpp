...
static char const ThisModelClass[] = "PowerTrain.PowerSupply";
...
MyModel_New(struct tInfos *Inf, void *pCfgIF, char const *KindKey, char const *IdKey)
{
    ...
    tPTPowerSupplyCfgIF *CfgIF = (tPTPowerSupplyCfgIF *) pCfgIF;
    ...
    if ((ModelKind = SimCore_GetKindInfo(Inf, ModelClass_PTPowerSupply, KindKey, 0, ThisVersionId, &VersionId))
        == NULL) {
        return NULL;
    }
    ...
    LV->Temp     = CfgIF->BattLV.Temp_init;
...
MyModel_Calc(void *MP, void *pIF, void *pIF2, double dt)
{
    ...
    tPTPowerSupplyIF  *IF = (tPTPowerSupplyIF *) pIF;
    ...
    /* Temperature */
    IF->BattLV.Temp         = LV->Temp;
    IF->BattLV.TempCool_out = IF->BattLV.TempCool_in;
