...
static void
MyModel_DeclQuants_dyn(struct tMyModel *mp, int park, char const *SuspModIdent)
{
    ...
    char Quant[40];
    char MdlPre[8];
    int  SuspModId = strtol(SuspModIdent, NULL, 10);

    Susp_SuspId2VhclPrefix(SuspModId, MdlPre);
...
MyModel_DeclQuants(void *MP, char const *SuspModIdent)
{
    ...
    if (mp == NULL) {
        ...
        MyModel_DeclQuants_dyn(mp, 0, SuspModIdent);
    }
}
...
MyModel_Delete(void *MP, char const *Ident)
{
    ...
    MyModel_DeclQuants_dyn(mp, 1, Ident);
