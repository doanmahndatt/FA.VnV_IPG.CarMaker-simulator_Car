...
/* int IO_None; DON'T define - Variable is predefined by CarMaker! */
...
void
iGetCal(tInfos *Inf, char const *key, tCal *cal, int optional)
{
    cal->Min       = 1e37f;
    cal->Max       = -1e37f;
    cal->LimitLow  = 1e37f;
    ...
    char const *item = iGetStrOpt(Inf, key, NULL);
    if (item != NULL) {
        int n = sscanf(item, "%g %g %g %g %d", &cal->LimitLow, &cal->LimitHigh, &cal->Factor, &cal->Offset,
            &cal->Rezip);
        if (n != 5) {
...
CalInF(tCal *cal, float Value)
{
    ...
    if (Result < cal->Min) {
        cal->Min = Result;
    } else if (Result > cal->Max) {
        cal->Max = Result;
    }

    if (Result < cal->LimitLow) {
        Result = cal->LimitLow;
    } else if (Result > cal->LimitHigh) {
        Result = cal->LimitHigh;
    }
...
# if defined(CM_HIL)
    ...
    if (nErrors != Log_nError) {
        LogErrF(EC_General, "MIO initialization failed. I/O disabled (2)");
        IO_SelectNone();
        return -1;
    }
# endif /* defined(CM_HIL) */
...
#if defined(CM_HIL)
    /*** Messages to the Fail Safe Tester */
    FST_MsgOut(CycleNo);
#endif  /* defined(CM_HIL) */
