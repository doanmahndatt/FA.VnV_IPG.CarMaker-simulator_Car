...
Vhcl_ModelCheck_BeforePre(struct tInfos *Inf)
{
    if (Vehicle.Model.Source != VehicleSource_BuiltIn) {
        LogErrF(EC_Init, "ModelCheck not supported for user-defined vehicles");
        goto ErrorReturn;
    }
...
Vhcl_Register(void)
{
    ...
    GenericPlugin_Register_ModifyDriverInput();

    VhclOperator_Register_IPGOperator();
    SelectorCtrlMapping_Register_Generic();

    ManAutoFollow_Register();
    ManAutoLaneChange_Register();
...
Vhcl_New(void)
{
    ...
    if (!SimCore.Vhcl.Modified) {
        ...
        if (Vehicle.Model.Source == VehicleSource_BuiltIn) {
            ...
            VhclModel_New(SimCore.Vhcl.Inf, 1);
        }
        if (SimCore.Trailer.nTrailers != 0) {
    ...
    if (Vehicle.Model.Source == VehicleSource_BuiltIn) {
        ...
        if (VhclModel_New(SimCore.Vhcl.Inf, 0) != 0) {
            rv |= 1 << 1;
        }
    } else {
...
Vhcl_Calc(double dt)
{
    ...
    TCPU_Section_TakeTS(SimCore.TS.Vehicle);
    ...
    if (SimCore.Trailer.nTrailers != 0) {
        ...
        TCPU_Section_TakeTS(SimCore.TS.Trailer);
    }
    ...
    TCPU_Section_TakeTS(SimCore.TS.Brake);
    ...
    TCPU_Section_TakeTS(SimCore.TS.PowerTrain);

    PowerFlow_Calc();

    /* Part of CockpitPackage Standard getting Torque from Vehicle.Steering.Trq as Force Feedback */
    DrivingSim_GetFeedback();
